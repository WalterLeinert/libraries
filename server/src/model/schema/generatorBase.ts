      ...
      switch (colInfo.type) {
        ...
        default:
          throw new NotSupportedException(
            util.format('unsupported data type: %s', DataType[colInfo.type as number]));
      }
  ...
  protected static mapDataType(type: DataType): string {
    switch (type) {
      ...
      case DataType.Date:
        return 'Date';
      default:
        throw new NotSupportedException(util.format('unsupported data type: %s', DataType[type as number]));
    }
  }
    ...
    using(new XLog(GeneratorBase.logger, levels.INFO, 'ctor', 'outputDir = %s', outputDir), (log) => {
      ...
      fs.stat(outputDir, (err, stats) => {
        if (!stats) {
          ...
          throw new ArgumentException('No directory: ' + outputDir);
        }
      });
    });
  }
