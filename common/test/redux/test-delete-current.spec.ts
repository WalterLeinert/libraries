...
class ReduxDeleteCurrentTest extends ReduxBaseTest<IUser, number, any> {
  ...
  @test 'should dispatch commands: ItemDeletedCommand'() {
    ...
    expect(state1).to.deep.equal({
      ...this.beforeState,
      items: this.beforeState.items.filter((item) => item.id !== ReduxDeleteCurrentTest.DELETE_ID),
      currentItem: null,
      deletedId: ReduxDeleteCurrentTest.DELETE_ID,
  ...
  protected before(done: (err?: any) => void) {
    super.before(() => {
      ...
      this.itemToDelete = state.items.filter((item) => item.id === ReduxDeleteCurrentTest.DELETE_ID)[0];

      // currentItem setzen -> nach update prüfen
      this.serviceRequests.setCurrent(this.itemToDelete);
      ...
      // Test: Item löschen
      this.serviceRequests.delete(ReduxDeleteCurrentTest.DELETE_ID);
